...
export default async function DashboardPage() {
    ...
    const dashboardTitle = isMirror ? "My Reflections" : "My Reputation Audits";
    const dashboardDesc = isMirror 
        ? "Manage your active and completed reflections." 
        : "Manage your active and completed 360-degree reviews.";
    const startNewBtnText = isMirror ? "Begin Reflection" : "Start New Audit";
    ...
    return (
        <div className="min-h-screen bg-background text-foreground p-4 py-12 md:p-12 transition-colors duration-300">
            <div className="max-w-4xl mx-auto flex flex-col md:flex-row items-start md:items-center justify-between mb-12 gap-4">
                <div>
                    <h1 className="text-4xl font-serif font-bold tracking-tight mb-2 text-foreground">{dashboardTitle}</h1>
                    <p className="text-muted-foreground">{dashboardDesc}</p>
                </div>
                <div className="flex items-center gap-3">
                    <LogoutButton />
                    <Link href="/dashboard/setup">
                        <Button className="rounded-full font-bold">{startNewBtnText}</Button>
                    </Link>
                </div>
            </div>
                ...
                {(!audits || audits.length === 0) ? (
                    <div className="text-center py-20 border border-dashed border-border rounded-lg bg-card/50">
                        <h3 className="text-xl font-serif font-medium mb-2 text-foreground">
                            {isMirror ? "No reflections found" : "No audits found"}
                        </h3>
                        <p className="text-muted-foreground mb-6">
                            {isMirror ? "You haven't requested any reflections yet." : "You haven't requested any feedback yet."}
                        </p>
                        <Link href="/dashboard/setup">
                            <Button variant="outline" className="border-border text-foreground hover:bg-secondary">
                                {isMirror ? "Setup your first reflection" : "Setup your first audit"}
                            </Button>
                        </Link>
                    </div>
                ) : (
                    audits.map((audit) => {
                        ...
                        return (
                            <Card key={audit.id} className="bg-card border-border text-card-foreground shadow-xl relative overflow-hidden">
                                {isUnlocked && (
                                ...
                                <CardHeader>
                                    <div className="flex items-center justify-between">
                                        <div>
                                            <CardTitle className="text-xl flex items-center gap-2 font-serif font-bold text-foreground">
                                        ...
                                        <Badge variant={completed === total ? "default" : "outline"} className={completed === total ? "bg-emerald-500/10 text-emerald-450 hover:bg-emerald-500/20" : "border-border text-muted-foreground"}>
                                    ...
                                    {!isUnlocked && (
                                        <div className="mt-4 space-y-2">
                                            <div className="flex justify-between text-xs font-medium">
                                                <span className="text-muted-foreground">{isMirror ? "Reflection Progress" : "Feedback Progress"}</span>
                                    ...
                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                        {audit.feedback_entries.map((entry: any) => (
                                            <div key={entry.id} className="p-3 rounded-md bg-secondary/40 border border-border/50 flex flex-col gap-2">
                                                ...
                                                <p className="text-xs text-muted-foreground">{entry.rater_name} ({entry.rater_email})</p>
                                            </div>
                                        ))}
                                    </div>
                                </CardContent>
                                    ...
                                    {isUnlocked ? (
                                        hasReport ? (
                                            <Link href={`/report/${audit.id}`} className="w-full">
                                                <Button className="w-full bg-secondary text-secondary-foreground hover:bg-secondary/80 font-bold border-none transition-colors">
                                                    View Final Report
                                                </Button>
                                            </Link>
                                        ) : (
                                        ...
                                        <Link href={`/dashboard/dispatch/${audit.id}`} className="w-full">
                                            <Button className="w-full font-bold rounded-full">
